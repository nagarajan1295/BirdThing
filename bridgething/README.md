...
1. Push this repo to `https://github.com/nagarajan1295/bridgething-app`.
2. In **Settings > Pages**, set the source to **Deploy from a branch**, branch `gh-pages`, folder `/ (root)`.

The catalog is published to `https://nagarajan1295.github.io/bridgething-app/catalog.v1.json`, which can be submitted to <bridgething.com/apps>
...
With more than one app in `apps/` your commands must specify which one: `bun run dev bridgething-app`.

Screenshot for the store listing:

```sh
bun run shot bridgething-app            # grabs what is on the screen
bun run shot bridgething-app --replace  # overwrite
```
...
```sh
bun run bump bridgething-app patch -m "Fix the wind direction arrow"
git commit -am "bridgething-app: fix the wind direction arrow" && git push
```
